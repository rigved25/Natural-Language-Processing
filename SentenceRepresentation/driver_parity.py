...
class ParityLSTM(torch.nn.Module) :
    ...
    def __init__(self, hidden_dim=8) :
        ...
        self.lstm = nn.LSTM(batch_first=True, input_size=1, hidden_size=hidden_dim, num_layers=1)
    ...
    def forward(self, x, s):
        packed_lstm_in = pack_padded_sequence(x, s, batch_first=True, enforce_sorted=False).to(dev)
        packed_lstm_out, (ht, ct) = self.lstm(packed_lstm_in)
...
def train_model(model, train_loader, epochs=500, lr=0.01):
