...
def main_glove():
	...
	learningRate = 0.11	# learning rate / step size for SGD
	...
	for epoch in range(maxEpoch):
		...
		# Start an epoch
		learningRate = learningRate - (0.01 if epoch % 4 == 0 else 0)
		...
		for b in range(idx.shape[0]//B+1):
			...
			for k in range(bSize):
				wordvecs_grad[k, :] = 2 * fval[k] * error[k] * c_batch[k, :]
				contextvecs_grad[k, :] = 2 * fval[k] * error[k] * w_batch[k, :]
