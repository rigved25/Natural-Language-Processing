...
def compute_cooccurrence_matrix(corpus, vocab):
	...
	for overview in corpus:
		tokens = vocab.tokenize(overview)
		for i, token in enumerate(tokens):
			if token not in vocab.word2idx.keys():
				continue

			for j in range(ctxt_size + 1):
				if i + j < len(tokens):
					if tokens[(i + j)] not in vocab.word2idx.keys():
						continue
				
					idx_i = vocab.word2idx[token]
					idx_j = vocab.word2idx[tokens[(i + j)]] 
...
def compute_ppmi_matrix(corpus, vocab):
	...
	P = np.zeros((vocab.size, vocab.size))
	for i, row in enumerate(C):
		for j, element in enumerate(row):
			if C[i][i] == 0 or C[j][j] == 0:
				pmi = 0
			else:
				pmi = (C[i][j] / C[i][i]) * (len(corpus) / C[j][j])
			
			if pmi == 0:
				P[i][j] = 0
			else:
				P[i][j] = np.log(max(1, pmi))
